...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const commentUsername = database.comments[id].username;
    const commentArticle = database.comments[id].articleId;

    //Delete commentId from article
    const indexCommentId = database.articles[commentArticle].commentIds.indexOf(id);
    database.articles[commentArticle].commentIds.splice(indexCommentId,1);

    //Delete commentId from user
    const indexUsername = database.users[commentUsername].commentIds.indexOf(id);
    database.users[commentUsername].commentIds.splice(indexUsername,1);

    //Delete comment from article — this is unnecessary because it's retrieved referentially
    //const indexComment = database.articles[commentArticle].comments.indexOf(id);
    //database.articles[commentArticle].comments.splice(indexComment,1);
